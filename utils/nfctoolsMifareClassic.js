...
const _read = () => {
  mc.isConnected({
    success: function (res) {
      ...
      accessData.readRes = []
      for (let i = 0; i < 4; i++) {
				console.log(accessData.rblock,'->i=',i)
				if (accessData.rblock !=999){
					i = accessData.rblock
				}
          ...
          fail: function (res) {
            console.log("读取 block 失败，使用密钥", accessData.keyType, "：", accessData.key, " 返回内容：", res)
				...
				if (accessData.rblock !=999){
					//break;
					return
					// block=999表明需要读取扇区下所有块，否则只读取指定块
				}
      }
      setTimeout(() => {
        _close()
